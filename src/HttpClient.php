...
final class HttpClient implements ClientInterface
{
    ...
    private function buildTransformers(): array
    {
        ...
        if ($this->config->baseUrl !== null) {
            $transformers[] = (new BaseUrl($this->uriFactory, $this->config->baseUrl))->__invoke(...);
        }
    ...
    {
        $transport = $transport ?? (new CurlTransport($this->responseFactory, $this->streamFactory))->__invoke(...);
